...
def sum_number(numb):
    tmp = str(numb)
    res = sum([int(tmp[i]) * (i + 1) for i in range(len(tmp))])
    return res


number = int(input('Введите целое многозначное число: '))
print('ОТВЕТ:\n\tВходное значение:\t\t\t\t', number,
      '\n\tСумма произведений каждой \n\tцифры на её порядковый номер:\t', sum_number(number))
